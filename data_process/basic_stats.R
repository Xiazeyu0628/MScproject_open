...
options()$scipen
rm(list=ls())
load('../my_models/Rdata/mturk/mturk_raw.Rdata')
library(dplyr) 
library(ggplot2)
theme_set(theme_bw())   # 设置绘图背景
rm(list=ls())   # 清除所有的变量
...
# Demographics
df.sw %>% count(sex)
ms(df.sw$age)   # $表示从一个dataframe中取出某一列数据
...
excluded_df = filter(df.sw.all, rule_like==0|bot==1) %>%   # filter筛选行
  group_by(condition) %>%   
  summarise(acc=sum(correct), n=n()) %>%
  mutate(acc=acc/(n*18), data='excluded')


approved_df = filter(df.sw.all, rule_like==1) %>% 
  group_by(condition) %>%
  summarise(acc=sum(correct), n=n()) %>%
...
binded_rows = bind_rows( excluded_df,  # 合并行
                         approved_df)
binded_rows_with_factor = mutate(binded_rows, data=factor(data, levels=c('excluded', 'approved')),
                                condition=factor(condition, levels=c('A1','A2','A3','A4')))

binded_rows_with_factor %>%
  ...
  scale_fill_brewer(palette="Paired")   # 调色板
